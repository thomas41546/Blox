...
QuadTree* QuadTree::getNextQuad(Entity* object)
{
...
bool QuadTree::quadFit(Entity *object)
{
	bool toReturn = true;
	Point2d<float> position = object->getPosition();
...
void QuadTree::eraseQuadObject(Entity *object)
{
	for(std::list<Entity *>::iterator it = m_objectList.begin(), e = m_objectList.end(); it != e; ++it)
	{
...
// recursive
bool QuadTree::erase(Entity* object)
{
	// if object fit in current quadtree
	if(quadFit(object))
	{
...
void QuadTree::getListCollision(Entity* object, std::list<Entity*> *toAppend)
{
	// append object
